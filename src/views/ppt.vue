<template>
  <column cross="space-between" style="overflow: hidden; width: 1000px">
    <row main="space-between">
      <ppt-title>{{ ppt.name }}</ppt-title>
      <row>
        <ppt-button :disabled="leftDisabled" @click="handlePage(-1)"
          >上一页</ppt-button
        >
        <padding p="0 10px"
          >{{ ppt.page }}/{{ Math.max(ppt.pages.length - 1, 0) }}</padding
        >
        <ppt-button :disabled="rightDisabled" @click="handlePage(1)"
          >下一页</ppt-button
        >
      </row>
    </row>
